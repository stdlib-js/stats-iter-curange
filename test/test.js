...
tape( 'main export is a function', function test( t ) {
	...
	t.equal( typeof itercurange, 'function', 'main export is a function' );
	t.end();
});
...
tape( 'the function returns an iterator protocol-compliant object which iteratively computes a cumulative range', function test( t ) {
	var expected;
	var actual;
	var values;
	var it;
	...
	it = itercurange( array2iterator( values ) );
...
tape( 'the function returns an iterator protocol-compliant object which iteratively computes a cumulative range (value+done)', function test( t ) {
	var expected;
	var actual;
	var values;
	var it;
	...
	it = itercurange( createIterator( values ) );
	t.equal( it.next.length, 0, 'has zero arity' );
...
tape( 'if an iterated value is a non-numeric value, the computed range is `NaN`', function test( t ) {
	var expected;
	var values;
	var it;
	var v;
...
tape( 'if an environment supports `Symbol.iterator` and the provided iterator is iterable, the returned iterator is iterable', function test( t ) {
	...
	itercurange = proxyquire( './../lib/main.js', {
